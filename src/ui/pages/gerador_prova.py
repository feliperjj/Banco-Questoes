...
class GeradorProvaPage(QWidget):
    def __init__(self):
        ...
        self.disciplina_input = QComboBox(); self.disciplina_input.setEditable(True); self.disciplina_input.addItem("Todas as disciplinas"); self.disciplina_input.addItems(repo.listar_disciplinas()); form.addRow("Filtro - Disciplina:", self.disciplina_input)
    ...
    def carregar_provas(self):
        ...
        for row, p in enumerate(provas):
            ...
            btn = QPushButton("Iniciar"); btn.setObjectName("table-action-button"); btn.setFixedSize(92, 30); btn.setToolTip("Iniciar prova"); btn.clicked.connect(lambda checked, pid=p["id"], linha=row: self.iniciar_prova(pid, linha))
