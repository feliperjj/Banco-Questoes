...
def processar_revisao(questao_id: int, acertou: bool):
    init_db()
    registro = RevisaoEspacada.get_by_id(questao_id)
    if not acertou:
    ...
    registro.proxima_revisao = datetime.date.today() + datetime.timedelta(days=intervalo)
    registro.save()


def questoes_para_revisar_hoje() -> list[dict]:
    init_db()
    query = (Questao.select(Questao, RevisaoEspacada.proxima_revisao)
             .join(RevisaoEspacada)
             .where(RevisaoEspacada.proxima_revisao <= datetime.date.today()))
    questoes = []
    for questao in query:
        revisao = RevisaoEspacada.get_by_id(questao.id)
